...
from photo_helpers import (
    get_photo_folder,
    save_photo,
    get_photo_path,
    delete_old_images,
    rename_photo_if_needed,
...
def derive_category_folder_from_db_path(db_path: str = None) -> str:
    """
    DEPRECATED: Derive category folder from database path.
    
    Now uses AppContext.photo_folder instead.
    Kept for backward compatibility only.
    """
    # This function is no longer used - centralized helpers handle this
    return ""
...
def get_photos_directory(category_folder: str = None) -> str:
    ...
    which reads from AppContext.photo_folder.
    """
...
def compress_and_save_image(source_path: str, target_path: str, max_size_mb: int = 5) -> bool:
    """
    DEPRECATED: Compress and save image with size limit.
    
    Use photo_helpers.compress_and_save_image() instead.
    Kept for backward compatibility.
    """
    # Import here to avoid circular dependency
    from photo_helpers import compress_and_save_image as centralized_save
    return centralized_save(source_path, target_path, max_size_mb)
