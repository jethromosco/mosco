...
class AdminPanel:
    ...
    def _get_product_form_handler_class(self, base_folder=None):
        ...
        Falls back to wipermono if not specified or import fails.
        
        Args:
            base_folder: Category folder name (e.g., 'packingseals.wipermono', 'oilseals')
                        Can be None for fallback to wipermono.
        
        Returns:
            ProductFormHandler class from the correct category module
        """
        if not base_folder:
            base_folder = 'packingseals.wipermono'
        ...
        # Fallback to wipermono if anything fails
        try:
            fallback_module = importlib.import_module('packingseals.wipermono.gui.gui_prod_aed')
    ...
    def create_products_tab(self):
        ...
        ProductFormHandlerClass = self._get_product_form_handler_class('packingseals.wipermono')
